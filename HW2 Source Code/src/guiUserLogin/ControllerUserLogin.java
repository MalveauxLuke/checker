...
public class ControllerUserLogin {
	...
	*/
	...
	protected static void doLogin(Stage ts) {
		theStage = ts;
		String identifier = ViewUserLogin.text_Username.getText(); // Changed variable name
		String password = ViewUserLogin.text_Password.getText();
		boolean loginResult = false;

		//LUKE: check if the input is an email
		boolean isEmail = identifier.matches("^[A-Za-z0-9+_.-]+@[A-Za-z0-9.-]+$");

		// Luke added: use appropriate lookup method based on input type
		boolean found;
		if (isEmail) {
			found = theDatabase.getUserAccountDetailsByEmail(identifier);
		} else {
			found = theDatabase.getUserAccountDetails(identifier);
		}

		// Fetch the user and verify the username/email
		if (!found) {
		...
		System.out.println("*** Username or Email is valid");
		...
		if (password.compareTo(actualPassword) != 0) {
    		...
    		boolean oneTimePassword = theDatabase.removeOneTimePasswordAfterUse(identifier, password);
    		if (!oneTimePassword) {
    			ViewUserLogin.alertUsernamePasswordError.setContentText(
        				"Incorrect username/password. Try again!");
        		ViewUserLogin.alertUsernamePasswordError.showAndWait();
		...
		// Use currentUsername from the database (important for email login)
		User user = new User(
