 ...
 */
	 ...
	 * 
	 * <p> Description: Given a username, this method creates an one time password
	 * and adds a record to the InvitationCodes table.  When the one time password is used, the
	 * record is removed from the table.</p>
	 * 
	 * @param username specifies the username of the user.
	 * 
	 * @return the password of 8 characters so the user can use it to enter his account.
	 * 
	 */
	// Generates a one time password and inserts it into the database.
	public String generateOneTimePassword(String username) {
	    String password = UUID.randomUUID().toString().substring(0, 8); // Generate a random 8-character oneTimePassword
	    String query = "INSERT INTO oneTimePasswords (password, username) VALUES (?, ?)";
	...
	/*******
	 * <p> Method: void removeOneTimePasswordAfterUse(String username, String password) </p>
	 * 
	 * <p> Description: Remove an one time password record once it is used.</p>
	 * 
	 * @param username is the String name of the Username
	 * 
	 * @param password is the 8 character String One Time Password code
	 * 
	 * @return boolean value depending whether One Time Password existed
	 *  
	 */
	// Remove a One Time Password once the user used it
	public boolean removeOneTimePasswordAfterUse(String username, String password) {
	    String query = "DELETE FROM oneTimePasswords WHERE password = ? AND username = ?";
	    try (PreparedStatement pstmt = connection.prepareStatement(query)) {
	        pstmt.setString(1, password);
	        pstmt.setString(2, username);
	...
	public String getPassword(String username) {
		...
		try (PreparedStatement pstmt = connection.prepareStatement(query)) {
	        ...
	        if (rs.next()) {
	            return rs.getString("password"); // Return the email address if user exists
	        }
	...
	/*******
	 * <p> Method: void updatePassword(String username, String emailAddress) </p>
	 * 
	 ...
	 * @param username is the username of the user
	 *  
	 * @param password is the new preferred first name for the user
	 *  
	 */
	// update the email address
	...
	//helper method. get all the attributes of a user given the user/s email address. This is used to support the login by email function. 
	// LUKE MALVEAUX DID THIS
	
	public boolean getUserAccountDetailsByEmail(String email) {
	...
	public void deleteUser(String username) throws SQLException { //had to use SQL for this because project is using H2 - mehmet 9/30/25
		...
		// used try for the error handling of this, learned from the other project code handling sql operations
	}
	
	public boolean checkUserDeleted(String username) { //mehmet 9/30/25
		return !doesUserExist(username); //super simple checks if a user still exists in the db
	...
	public String[] listUserDetails() { //mehmet 9/24/25
		List<String> detailOfUser = getUserList(); //im using this list to grab the usernames
		
		String[] userInfo = new String[detailOfUser.size()]; //userInfo is gonna store all the info on the user
		
		for (int i = 0; i < detailOfUser.size(); i++) {
			String selectedUsername = detailOfUser.get(i);
			if (i == 0) { //i hated that getUserList had a default <Select a user> string so I just made sure on the first iteration of the for loop to replace it with a header of some sort.
				userInfo[i] = "ACTIVE USERS LIST: \n";
				
			} else {
				getUserAccountDetails(selectedUsername); //this method gave all the info
				
				String firstName = getFirstName(selectedUsername);
				if (firstName.isEmpty()) { firstName = "N/A";} //if condition if they never input the info default to N/A
				String lastName = getLastName(selectedUsername);
				if (lastName.isEmpty()) { lastName = "N/A";} //if condition if they never input the info default to N/A
				String emailAddr = getEmailAddress(selectedUsername);
				
				String userRoles = "";
				
				//ROLE TYPES
				if(getCurrentAdminRole()) { userRoles += "Admin ";}
				if(getCurrentNewRole1()) { userRoles += "Role1 ";} //student to be defined role1?
				if(getCurrentNewRole2()) { userRoles += "Role2 ";} //staff to be defined role2?
				
				userInfo[i] = "Username: " + selectedUsername + "  " + "Firstname: " + firstName + "  " + "Lastname: " + lastName + "  " + "Email: " + emailAddr + "  " + "Active roles: " + userRoles + "\n";
