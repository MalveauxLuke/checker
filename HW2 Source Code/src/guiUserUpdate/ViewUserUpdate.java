...
public class ViewUserUpdate {
	...
	//  This alert is used when the user enter updated password that does not satisfy the requirements
	...
	private ViewUserUpdate() {
    	...
    	String passwordErr = passwordPopUpWindow.Model.evaluatePassword(result.get());
    	if (passwordErr == "") {
			...
			alertUsernamePasswordError.setContentText(passwordErr);
        ...
        // Email Address
        // Luke malveaux edit: added a check before the user changes the email to ensure the email is correct.
        ...
        button_UpdateEmailAddress.setOnAction((event) -> {
            ...
            result.ifPresent(name -> {
            	//the same check as our other email validation method
                if (name.matches("^[A-Za-z0-9+_.-]+@[A-Za-z0-9.-]+$")) {
                    ...
                    //easier than creating a popup box (idk how to use java)
                } else {
                    label_CurrentEmailAddress.setText("Invalid Email");
                }
            });
        });
